...
public class Program01 {
	...
	public static void main(String[] args) {
		...
		int n,i=0;
		int choice;
		System.out.println(" Enter Number Of Employee:");//3 
		n=sc.nextInt( );
		...
		while((choice=menu(sc))!=7) {
			...
			case 1:
					if(i<n) {
						...
						Date dob=new Date(bday,bmonth,byear);
						...
						Date doj=new Date(dday,dmonth,dyear);
						
						if(emp[i]==null) {
							emp[i]=new Employee(name,dob,id, dept, salary, doj);
							i++;
						}
					}else {
			...
			case 4:
					...
					for( int j=0;j<emp.length;j++) {
						if(emp[j]!=null && eyr==emp[j].getDoj().getYear()) {
							System.out.println(emp[j]);
							yfound=true;
						}
					}
			...
			case 5:
					...
					Employee e=emp[0];
					for( int j=0;j<emp.length;j++) {
						if(emp[j]!=null && e.getSalary()<emp[j].getSalary()) {
							e=emp[j];						}
					}
			...
			case 6:
					...
					Employee e1=emp[0];
					for( int j=0;j<emp.length;j++) {
						if(emp[j]!=null && e1.getSalary()>emp[j].getSalary()) {
							e1=emp[j];					
							}
					}
